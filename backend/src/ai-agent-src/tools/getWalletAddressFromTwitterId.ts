...
async function getWalletAddressFromTwitterIdHandler(twitterId: string): Promise<any> {
    ...
    try {
        ...
        console.log(`wallet address for your twitterId @${twitterId} is: ${cdpWalletAddress || ethereumWalletPublicKey}, uuid is ${response1.data.useruuid}`);


        return `wallet address for your twitterId @${twitterId} is: ${cdpWalletAddress || ethereumWalletPublicKey}, uuid is ${response1.data.useruuid}` as any;
